import { PostData } from './storage-manager';
...
export class PostExtractor {
  extractPostData(postElement: Element): PostData | null {
    try {
      ...
      const postUrl = window.location.href.split('?')[0] + '?utm_source=croi_extension';
      ...
      console.log('Croi Extension: Extracted post data:', result);
      return result;
    } catch (error) {
